# Apex Camera — Aplicación de Fotografía y Grabación de Vídeo Profesional para Android
...
* **Modo IA Pro (Zero-DCE Neural Tone Mapping en C++20):** Motor de inteligencia artificial local que evalúa dinámicamente la iluminación y contraste de la toma. Aplica estimación no lineal de curvas múltiples para rescatar sombras profundas sin ruido ni grano, comprime altas luces para evitar cielos quemados y resalta micro-texturas orgánicas protegiendo los tonos de piel en espacio YCbCr. Se activa mediante el botón directo `[IA Pro]` en el visor o con 1 toque en la previsualización de fotos.
* **Marca de Agua Inteligente Adaptativa ("SHOT ON APEX CAMERA"):** Sistema de estampa óptica profesional que permite certificar y comparar los resultados visuales de Apex Camera frente a la cámara nativa del teléfono. Adapta inteligentemente su posición, proporciones y márgenes de seguridad según la relación de aspecto elegida (**16:9**, **Full**, **4:3**, **1:1**) para no estorbar al sujeto principal ni colisionar con esquinas redondeadas o barras de navegación. Puede activarse/desactivarse tanto en el menú de Configuración como directamente en el **panel de confirmación / visualización de fotos**, permitiendo estampar fotos al instante.
* **Visor Inmersivo y Filtros en Tiempo Real:** Integración nativa con CameraX `Preview`, `ImageCapture` y `VideoCapture<Recorder>` de latencia reducida, con previsualización en vivo de los perfiles de color y contraste en el visor.
* **Selector de Relación de Aspecto (Full, 16:9, 4:3, 1:1):** Selector dinámico en la botonera superior y en Ajustes que permite componer el encuadre exacto antes de disparar. Soporta modo **Full** (pantalla completa del dispositivo), **16:9** (panorámico estándar para historias y estados), **4:3** (sensor nativo al 100% sin recorte de megapíxeles) y **1:1** (cuadrado con máscara visual en vivo para perfiles y redes sociales), con procesamiento central en `Dispatchers.IO`.
...
* **Control de Filtros e Intensidad en Ajustes y Barra Superior:** Botón de acceso directo (`[Belleza / Belleza OFF]`) y selector flotante de filtros fotográficos en la barra superior en modo foto, con sección dedicada en Ajustes para conmutar manualmente entre Vulkan 1.1 y OpenGL ES 3.2, y calibrar la intensidad del suavizado (20% al 100%).
...
* **Grabación de Vídeo de Alto Rendimiento y HDR Resiliente:** Soporte completo para captura de vídeo con guardado en `Movies/Camara` en formato MP4 vía `MediaStore`. Soporte multi-canal para HDR (perfiles 10-bit HLG/HDR10 en hardware compatible, con fallback adaptativo por Camera2 Interop para evitar bloqueos artificiales).
* **Calibración de Color y Contraste (Anti-Colores Lavados/Pasteles):** Pantalla especializada (`ColorCalibrationScreen`) para solucionar la sobreexposición y aplanamiento de sombras común en sensores OEM. Permite aplicar **Modo Antilavado Automático (-0.7 EV)**, compensación manual de exposición EV continua y perfiles de color (*Vívido Antilavado*, *Alto Contraste Cinematográfico*, *Cálido Natural*, *B&N Artístico*, *Estándar*).
* **Panel de Confirmación / Visualización Enriquecido:** Pantalla de visualización a pantalla completa con zoom interactivo, control y activación rápida de la marca de agua Apex Camera ("Estampar en esta foto"), compartir y borrado seguro.
